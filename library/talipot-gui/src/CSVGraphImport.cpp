                ...
                for (uint i : elements.second) {
                    ...
                    if (!(isVectorProperty
                              ? static_cast<VectorPropertyInterface *>(property)
                                    ->setNodeStringValueAsVector(node(i), tokens[column])
                              : property->setNodeStringValue(node(i), tokens[column][0]))) {
                        // We add one to the row number as in the configuration widget we start from
                        // row 1 not row 0
                        qWarning() << __PRETTY_FUNCTION__ << ":" << __LINE__
                                   << " error when importing token \"" << lineTokens[column]
                                   << "\" in property \"" << property->getName() << "\" of type \""
                                   << property->getTypename() << "\" at line " << row + 1;
                    }
                }
            } else {
                for (uint i : elements.second) {
                    if (!(isVectorProperty
                              ? static_cast<VectorPropertyInterface *>(property)
                                    ->setEdgeStringValueAsVector(edge(i), tokens[column])
