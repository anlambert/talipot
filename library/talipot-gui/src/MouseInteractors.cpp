    ...
    if (e->type() == QEvent::Gesture) {
        ...
        if (gesture->gesture(Qt::PinchGesture)) {
            ...
            if (pinch->changeFlags() & QPinchGesture::ScaleFactorChanged) {
                // only set the zoom factor if two events in a row were in the same direction (zoom
                // in or out) to smooth a bit the effect.
            ...
            if (pinch->changeFlags() & QPinchGesture::RotationAngleChanged) {
                // rotates the camera
                camera.rotate((pinch->rotationAngle() - pinch->lastRotationAngle()) / 180 * M_PI, 0,
                              0, 1);
            }
...
bool MouseMove::eventFilter(QObject *widget, QEvent *e) {
    ...
    if (e->type() == QEvent::MouseMove) {
        ...
        if (qMouseEv->pos().x() != x) {
            glWidget->scene()->translateCamera(glWidget->screenToViewport(qMouseEv->pos().x() - x),
                                               0, 0);
        }

        if (qMouseEv->pos().y() != y) {
            glWidget->scene()->translateCamera(
                0, glWidget->screenToViewport(y - qMouseEv->pos().y()), 0);
        }
...
bool MouseNKeysNavigator::eventFilter(QObject *widget, QEvent *e) {
    ...
    if (e->type() == QEvent::MouseButtonDblClick && qMouseEv->button() == Qt::LeftButton) {
        ...
        if (qMouseEv->modifiers() != Qt::ControlModifier) {
            ...
            glWidget->pickNodesEdges(qMouseEv->pos().x() - 1, qMouseEv->pos().y() - 1, 3, 3,
                                     tmpNodes, tmpEdges);
            node metaNode;
            bool find = false;
            ...
            if (!graphHierarchy.empty() && nldv) {
                ...
                glWidget->scene()->getLayer("Main")->getCamera().setSceneRadius(
                    camera.getSceneRadius());
                glWidget->scene()->getLayer("Main")->getCamera().setUp(camera.getUp());
                glWidget->scene()->getLayer("Main")->getCamera().setZoomFactor(
