...
static void restoreTextureFilesFromProject(tlp::Graph *g, tlp::Project *project,
    ...
    // Get a stable iterator on non default valuated edges as their value can be reset to the
    // default one by the possible call to setAllEdgeValue afterwards
...
QMap<QString, tlp::Graph *> GraphHierarchiesModel::readProject(tlp::Project *project,
    ...
    for (const QString &entry :
        ...
        for (const QString path :
             {"/graph.tlp", "/graph.tlp.zst", "/graph.tlpb", "/graph.tlpb.zst"}) {
...
QMap<tlp::Graph *, QString> GraphHierarchiesModel::writeProject(tlp::Project *project,
    ...
    for (auto *g : _graphs) {
        ...
        if (!Settings::isUseTlpbFileFormat()) {
            ...
            tlp::saveGraph(g,
                           QStringToTlpString(project->toAbsolutePath(folder + "graph.tlpb.zst")),
                           progress);
        }
    }
