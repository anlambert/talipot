...
class CustomTabBar : public QTabBar {
  ...
  protected:
    QSize tabSizeHint(int index) const {
        ...
        return QSize(width,
                     fontMetrics().horizontalAdvance(tabText(index)) * 2 + iconSize().width());
    }
};
...
WorkspacePanel::WorkspacePanel(tlp::View *view, QWidget *parent)
    ...
    connect(_ui->linkButton, &QAbstractButton::toggled, this,
            &WorkspacePanel::toggleSynchronization);
...
void WorkspacePanel::refreshInteractorsToolbar() {
    ...
    if (interactorsUiShown) {
        ...
        for (auto *i : compatibleInteractors) {
            ...
            connect(i->action(), &QAction::triggered, this,
                    &WorkspacePanel::interactorActionTriggered);
...
void WorkspacePanel::viewGraphSet(tlp::Graph *g) {
    ...
    if (g) {
        tlp::debug() << "Setting graph " << g->getName() << " for panel "
                     << windowTitle().toStdString() << std::endl;
    }
