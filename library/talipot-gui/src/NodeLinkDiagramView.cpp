...
void NodeLinkDiagramView::updateGrid() {
    ...
    BoundingBox graphBB = computeBoundingBox(graph(), inputData->layout(), inputData->sizes(),
                                             inputData->rotations());
    ...
    glWidget()->scene()->getLayer("Main")->addGlEntity(_grid, "Node Link Diagram Component grid");
}
...
void NodeLinkDiagramView::setState(const tlp::DataSet &data) {
    ...
    glWidget()->setKeepScenePointOfViewOnSubgraphChanging(keepSPOV);
...
void NodeLinkDiagramView::graphChanged(tlp::Graph *graph) {
    ...
    if (oldGraph == nullptr || graph == nullptr || (oldGraph->getRoot() != graph->getRoot()) ||
        !glWidget()->keepScenePointOfViewOnSubgraphChanging()) {
        centerView();
    }
...
void NodeLinkDiagramView::loadGraphOnScene(Graph *graph) {
    ...
    if (oldGlGraph->inputData()->graph() == graph) {
        ...
        glGraph->inputData()->setGlVertexArrayManager(
            oldGlGraph->inputData()->glVertexArrayManager());
...
void NodeLinkDiagramView::setZOrdering(bool f) {
    glWidget()->renderingParameters().setElementZOrdered(f);
    centerView();
}
...
void NodeLinkDiagramView::fillContextMenu(QMenu *menu, const QPointF &point) {
    ...
    result = glWidget()->pickNodesEdges(point.x(), point.y(), entity);
    ...
    if (result) {
        ...
        if (isNode) {
            ...
            action = selectMenu->addAction("edge and its extremities",
                                           [this] { selectEdgeAndExtremities(); });
        ...
        if (isNode) {
            ...
            action = addToSelectionMenu->addAction("predecessor nodes",
                                                   [this] { addInNodesToSelection(); });
            action->setToolTip("Add the predecessors of node #" + sId +
                               " to the current selection");
            action = addToSelectionMenu->addAction("successor nodes",
                                                   [this] { addOutNodesToSelection(); });
            action->setToolTip("Add the successors of node #" + sId + " to the current selection");
            action =
                addToSelectionMenu->addAction("input edges", [this] { addInEdgesToSelection(); });
            action->setToolTip("Add the input edges of node #" + sId + " to the current selection");
            action =
                addToSelectionMenu->addAction("output edges", [this] { addOutEdgesToSelection(); });
            action->setToolTip("Add the output edges of node #" + sId +
                               " to the current selection");
        ...
        if (isNode) {
            action =
                removeFromSelectionMenu->addAction("node", [this] { removeItemFromSelection(); });
            ...
            action =
                removeFromSelectionMenu->addAction("edge", [this] { removeItemFromSelection(); });
            action->setToolTip("Remove the edge #" + sId + " from the current selection");
            action = removeFromSelectionMenu->addAction(
                "edge extremities", [this] { removeExtremitiesFromSelection(); });
            action->setToolTip("Remove the extremities of edge #" + sId +
                               " from the current selection");
            action = removeFromSelectionMenu->addAction(
        ...
        if (isNode) {
            ...
            action = toggleMenu->addAction("predecessor nodes",
                                           [this] { addRemoveInNodesToSelection(); });
            action->setToolTip("Invert the selection of the predecessors of the node #" + sId);
            action = toggleMenu->addAction("successor nodes",
                                           [this] { addRemoveOutNodesToSelection(); });
            action->setToolTip("Invert the selection of the successors of the node #" + sId);
            action =
                toggleMenu->addAction("input edges", [this] { addRemoveInEdgesToSelection(); });
            action->setToolTip("Invert the selection of the input edges of the node #" + sId);
            action =
                toggleMenu->addAction("output edges", [this] { addRemoveOutEdgesToSelection(); });
            ...
            action = toggleMenu->addAction("edge extremities",
                                           [this] { addRemoveExtremitiesToSelection(); });
            action->setToolTip("Invert the selection of the source and target nodes of the edge #" +
                               sId);
        ...
        if (isNode) {
            ...
            if (metaGraph) {
                action = menu->addAction(FontIcon::icon(MaterialDesignIcons::LocationEnter),
                ...
                menu->addAction(FontIcon::icon(MaterialDesignIcons::Ungroup), "Ungroup",
                                [this] { ungroupItem(); });
                action->setToolTip("Replace the meta-node #" + sId +
        ...
        QAction *action = menu->addAction(FontIcon::icon(MaterialDesignIcons::LayersTripleOutline),
                                          "Use Z ordering");
        action->setToolTip(
        ...
        action = menu->addAction(FontIcon::icon(MaterialDesignIcons::Grid),
...
void NodeLinkDiagramView::addRemoveItemToSelection(bool pushGraph, bool toggleSelection,
    ...
    if (isNode) {
        elementSelected->setNodeValue(node(itemId),
                                      toggleSelection ? !elementSelected->getNodeValue(node(itemId))
                                                      : selectValue);
    } else {
        elementSelected->setEdgeValue(edge(itemId),
                                      toggleSelection ? !elementSelected->getEdgeValue(edge(itemId))
                                                      : selectValue);
    }
}
    ...
    for (auto neigh : graph()->getInNodes(node(itemId))) {
        if (!inNodes.get(neigh.id)) {
            elementSelected->setNodeValue(
                neigh, toggleSelection ? !elementSelected->getNodeValue(neigh) : selectValue);
...
void NodeLinkDiagramView::editValue(PropertyInterface *pi) {
    ...
    QVariant val = ItemDelegate::showEditorDialog(isNode ? ElementType::NODE : ElementType::EDGE,
                                                  pi, graph(), &tid, glWidget(), itemId);
...
void NodeLinkDiagramView::editColor() {
    editValue(glWidget()->inputData()->colors());
}

void NodeLinkDiagramView::editLabel() {
    editValue(glWidget()->inputData()->labels());
}

void NodeLinkDiagramView::editShape() {
    editValue(glWidget()->inputData()->shapes());
}

void NodeLinkDiagramView::editSize() {
    editValue(glWidget()->inputData()->sizes());
}
...
const Camera &NodeLinkDiagramView::goInsideItem(node meta) {
    ...
    centerView();
    draw();
    return glWidget()->scene()->getLayer("Main")->getCamera();
}
