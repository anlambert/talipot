...
void CaptionGraphicsBackgroundItem::updateCaption(float begin, float end) {
    ...
    if (!_interactionsActivated) {
        _minTextItem->setPos(QPointF(_captionContentPos +
                                     QPoint(17 - _minTextItem->boundingRect().width() / 2., 157)));
        _maxTextItem->setPos(QPointF(_captionContentPos +
                                     QPoint(17 - _maxTextItem->boundingRect().width() / 2., -22)));
...
void MovablePathItem::updatePath() {
    ...
    for (const auto &it : _metricToSizeFilteredList) {
        if (it.first < firstLimit) {
            ...
            if (state == 2) {
                // init of third path
                float midValue = computeToto(secondLimit, lastValue.second, it.second,
