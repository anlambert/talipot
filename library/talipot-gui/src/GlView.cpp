    ...
    if (needQuickAccessBar) {
        if (_showQabButton == nullptr) {
            ...
            connect(_showQabButton, &QAbstractButton::toggled, this,
                    &GlView::setQuickAccessBarVisible);
        }
...
void GlView::fillContextMenu(QMenu *menu, const QPointF &pf) {
    ...
    if (selection && (selection->hasNonDefaultValuatedNodes(graph()) ||
        ...
        QAction *znpOnSelection = menu->addAction(
            FontIcon::icon(MaterialDesignIcons::MagnifyExpand), "Zoom and pan on selection");
    ...
    QAction *viewOrtho = menu->addAction(FontIcon::icon(MaterialDesignIcons::AxisArrow),
...
bool GlView::pickNodeEdge(const int x, const int y, node &n, edge &e, bool pickNode,
    ...
    bool foundEntity =
        glWidget()->pickNodesEdges(x, y, selectedEntity, nullptr, pickNode, pickEdge);
    n = node();
    e = edge();
    if (foundEntity) {
