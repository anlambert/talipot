...
QString getPluginPackageName(const QString &pluginName) {
    return pluginName.simplified().remove(' ').toLower() + "-" + TALIPOT_VERSION + "-" +
    ...
    bool loadTexture(const std::string &filename, GlTexture &glTexture) override {
        ...
        if (qFilename.startsWith("http")) {
            ...
            if (imageData.isEmpty()) {
                ...
                if (!imageLoaded) {
                    tlp::error() << "Error when loading texture from url " << filename.c_str()
                                 << std::endl;
                    return false;
                }
            }
...
void qDetailedMessageBox(QMessageBox::Icon icon, const QString &title, const QString &text,
    ...
    if (!detailedText.isEmpty()) {
        ...
        if (textBoxes.size()) {
            ...
            textBoxes[0]->setFixedHeight(
                min(int(splitText.size() + 1) * msgBox.fontMetrics().height(),
