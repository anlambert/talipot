...
bool GraphModel::setAllNodeValue(PropertyInterface *prop, QVariant v, const Graph *graph) {
    if (dynamic_cast<IntegerProperty *>(prop) != nullptr) {
        ...
        if (prop->getName() == "viewFont") {
            ...
            static_cast<StringProperty *>(prop)->setAllNodeValue(
                QStringToTlpString(v.value<QString>()), graph);
        }
    } else if (dynamic_cast<BooleanVectorProperty *>(prop) != nullptr) {
...
bool GraphSortFilterProxyModel::lessThan(const QModelIndex &left, const QModelIndex &right) const {
    ...
    return graphModel->lessThan(graphModel->elementAt(left.row()),
                                graphModel->elementAt(right.row()),
...
void GraphModel::treatEvents(const std::vector<tlp::Event> &) {
    ...
    for (const PUB &e : _elementsToModify) {
        ...
        if (add) {
            ...
            // meaning its index in the model will be contiguous with the one of the last added
            // element. So add it to the current rows sequence that will be further added in the
            // model
            if (_elements.empty() || id > uint(_elements.back())) {
                rowsSequence.push_back(id);
            }
            // case where an element previously deleted, whose id is lower than the last one stored
            // in the model, is added again in the graph
            else {
            ...
            // if the index to remove is not contiguous with the last one stored in the current
            // sequence of indices to remove, flush that sequence to remove the elements from the
            // model
...
void NodesGraphModel::treatEvent(const Event &ev) {
    ...
    if (graphEv) {
        ...
        if (graphEv->getType() == GraphEventType::TLP_ADD_NODE) {
            ...
            // Observable::unholdObservers(), remove it from the elementsToModify list as no update
            // has to be performed in the model for that element.
            int wasDeleted =
                _nodesRemoved ? _elementsToModify.indexOf(qMakePair(graphEv->getNode().id, false))
                              : -1;
            ...
            for (auto n : graphEv->getNodes()) {
                // if the node was removed then added again before the call to
                // Observable::unholdObservers(), remove it from the elementsToModify list as no
                // update has to be performed in the model for that element
                int wasDeleted =
                    _nodesRemoved ? _elementsToModify.indexOf(qMakePair(n.id, false)) : -1;
            ...
            _nodesRemoved = true;
            // if the node was added then deleted before the call to Observable::unholdObservers(),
            // remove it from the elementsToModify list as no update has to be performed in the
            // model for that element
            int wasAdded = _nodesAdded
                               ? _elementsToModify.indexOf(qMakePair(graphEv->getNode().id, true))
                               : -1;
...
void EdgesGraphModel::treatEvent(const Event &ev) {
    ...
    if (dynamic_cast<const GraphEvent *>(&ev) != nullptr) {
        ...
        if (graphEv->getType() == GraphEventType::TLP_ADD_EDGE) {
            ...
            // has to be performed in the model for that element
            int wasDeleted =
                _edgesRemoved ? _elementsToModify.indexOf(qMakePair(graphEv->getEdge().id, false))
                              : -1;
            ...
            for (auto e : graphEv->getEdges()) {
                ...
                // update has to be performed in the model for that element
                int wasDeleted =
                    _edgesRemoved ? _elementsToModify.indexOf(qMakePair(e.id, false)) : -1;
            ...
            _edgesRemoved = true;
            // if the edge was added then deleted before the call to Observable::unholdObservers(),
            // remove it from the elementsToModify list as no update has to be performed in the
            // model for that element
            int wasAdded = _edgesAdded
                               ? _elementsToModify.indexOf(qMakePair(graphEv->getEdge().id, true))
                               : -1;
