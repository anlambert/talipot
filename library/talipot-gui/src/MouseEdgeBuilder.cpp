...
bool MouseEdgeBuilder::eventFilter(QObject *widget, QEvent *e) {
    ...
    if (e->type() == QEvent::MouseButtonPress) {
        ...
        if (qMouseEv->buttons() == Qt::LeftButton) {
            if (!_started) {
                bool result = glWidget->pickNodesEdges(qMouseEv->pos().x(), qMouseEv->pos().y(),
                                                       selectedEntity);
    ...
    if (e->type() == QEvent::MouseMove) {
        ...
        if (!_started) {
            SelectedEntity selectedEntity;
            bool hoveringOverNode = glWidget->pickNodesEdges(qMouseEv->pos().x(),
...
void MouseEdgeBuilder::treatEvent(const Event &evt) {
    if (typeid(evt) == typeid(GraphEvent)) {
        ...
        if (propertyEvent &&
            propertyEvent->getType() == PropertyEventType::TLP_AFTER_SET_NODE_VALUE &&
