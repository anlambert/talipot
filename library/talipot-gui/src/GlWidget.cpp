...
void GlWidget::createFramebuffers(int width, int height) {
    ...
    if (!_glFrameBufAntialiased || _glFrameBufAntialiased->size().width() != width ||
        ...
        GlTextureManager::registerExternalTexture(_sceneTextureId,
                                                  _glFrameBufSceneTexture->texture());
...
void GlWidget::pickNodesEdges(const int x, const int y, const int width, const int height,
    ...
    if (pickNodes) {
        _scene.selectEntities(
            ...
            screenToViewport(height), layer, selectedNodes);
    }
    ...
    if (pickEdges) {
        _scene.selectEntities(
            ...
            screenToViewport(x), screenToViewport(y), screenToViewport(width),
            screenToViewport(height), layer, selectedEdges);
    }
