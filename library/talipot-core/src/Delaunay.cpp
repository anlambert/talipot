...
static tlp::Coord computeTriangleCircumscribedCenter(const tlp::Coord &A, const tlp::Coord &B,
    ...
    if (axbnorm != 0) {
        Vec3ld c =
            Cd + (((anorm * anorm * b) - (bnorm * bnorm * a)) ^ axb) / (2.0 * axbnorm * axbnorm);
        ...
        if (voronoiMode) {
            ...
            for (size_t i = 0; i < hullPoints.size() - 1; ++i) {
                for (float j = 0; j < nbSubdiv - 1; ++j) {
                    newHullPoints.push_back(
                        hullPoints[i] + (j / (nbSubdiv - 1)) * (hullPoints[i + 1] - hullPoints[i]));
                }
            }
    ...
    if (ret) {
        ...
        for (const auto &simplex : simplices) {
            ...
            if (face4.isValid()) {
                ...
                if (const auto it = faceToCircumCenter.find(face4);
                    it != faceToCircumCenter.end()) {
                    addVoronoiEdge(voronoiDiagram, face4,
                                   {circumCenterIdx, faceToCircumCenter[face4]});
                } else {
