        ...
        if (!norm) {
            ...
            case EdgeType::UNDIRECTED:
                TLP_PARALLEL_MAP_NODES(
                    graph, [&](const node n) { deg[n] = normalization * graph->deg(n); });
                break;

            case EdgeType::INV_DIRECTED:
                TLP_PARALLEL_MAP_NODES(
                    graph, [&](const node n) { deg[n] = normalization * graph->indeg(n); });
                break;

            case EdgeType::DIRECTED:
                TLP_PARALLEL_MAP_NODES(
                    graph, [&](const node n) { deg[n] = normalization * graph->outdeg(n); });
                break;
            }
        }
    } else {
