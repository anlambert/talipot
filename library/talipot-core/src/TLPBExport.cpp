...
bool TLPBExport::exportGraph(std::ostream &os) {
                    ...
                    if (!beginNode.isValid()) {
                        ...
                        if (current.id == lastNode.id + 1) {
                            ...
                            if (rangesToWrite == MAX_RANGES_TO_WRITE) {
                                vRangesVec.push_back(
                                    std::vector<std::pair<node, node>>(MAX_RANGES_TO_WRITE));
                ...
                for (uint j = 0; j < nbEdges; ++j) {
                    ...
                    if (!beginEdge.isValid()) {
                        ...
                        if (current.id == lastEdge.id + 1) {
                            ...
                            if (rangesToWrite == MAX_RANGES_TO_WRITE) {
                                vRangesVec.push_back(
                                    std::vector<std::pair<edge, edge>>(MAX_RANGES_TO_WRITE));
                                vRanges = vRangesVec.back();
                                rangesToWrite = 0;
                                pendingWrite = false;
                            }
    ...
    // write properties
    {
        ...
        for (uint i = 0; i < numProperties; ++i) {
            ...
            bool pnViewProp =
                (nameOrType == string("viewFont") || nameOrType == string("viewTexture"));
            // write type
            ...
            // write nodes values
            {
                ...
                if (valueSize && canUsePubSetBuf) {
                    ...
                    vBuf = static_cast<char *>(
                        malloc(MAX_VALUES_TO_WRITE * (sizeof(uint) + valueSize)));
