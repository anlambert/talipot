...
void TreeTest::makeRootedTree(Graph *graph, node root) {
    ...
    if (!TreeTest::isFreeTree(graph)) {
        tlp::warning() << "makeRootedTree: The graph is not topologically a tree, so rooted tree "
                          "cannot be made."
                       << endl;
        return;
    } // end if
...
#define REVERSED_EDGES "ReversedEdges"
    ...
    if (isConnected || ConnectedTest::isConnected(gClone)) {
        ...
        return computeTreeInternal(gClone->addSubGraph(&treeSelection), rGraph, true,
                                   pluginProgress, reversedEdges);
    }
