...
Observable::~Observable() {
    ...
    TLP_GLOBALLY_LOCK_SECTION(ObservableGraphUpdate) {
        ...
        if (!observationGraph.alive[_n.id]) {
            tlp::error() << "[ERROR]: in " << __PRETTY_FUNCTION__ << " at " << __FILE__ << ':'
                         << __LINE__ << endl;
            tlp::error() << "Observable object has already been deleted, possible double free!!!"
                         << endl;
            std::terminate();
        }
...
void Observable::observableDeleted() {
    ...
    if (hasOnlookers()) {
        Event msg(*this, EventType::TLP_INVALID); // create a modify event to prevent raise
