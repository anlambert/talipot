...
bool TLPBImport::importGraph() {
    ...
    subgraphs[0] = graph;
    {
        ...
        if (!bool(inputData.is->read(reinterpret_cast<char *>(&numSubGraphs),
                                     sizeof(numSubGraphs)))) {
            return false;
        }
        ...
        for (uint i = 0; i < numSubGraphs; ++i) {
            ...
            // read sg nodes ranges
            {
                ...
                if (!bool(inputData.is->read(reinterpret_cast<char *>(&numRanges),
                                             sizeof(numRanges)))) {
                    return false;
                }
    ...
    // read properties
    {
        ...
        if (!bool(inputData.is->read(reinterpret_cast<char *>(&numProperties),
        ...
        for (uint i = 0; i < numProperties; ++i) {
            ...
            // nodes / edges values
            {
                ...
                if (!bool(inputData.is->read(reinterpret_cast<char *>(&numValues),
                                             sizeof(numValues)))) {
                    return false;
                }
                ...
                // for backward compatibility with TLPB format <= 1.1, (see sourceforge commit
                // #11536)
                ...
                if (size && canUsePubSetBuf) {
                    ...
                    for (uint i = 0; i < numValues; ++i) {
                        ...
                        // read node id
                        if (!bool(inputData.is->read(reinterpret_cast<char *>(&(n.id)),
                                                     sizeof(uint)))) {
                            return false;
                        }
                        ...
                        if (pnViewProp) {
                            ...
                            // if needed replace symbolic path by real path
                            if (size_t pos = value.find(TalipotBitmapDirSym);
                                pos != std::string::npos) {
                                value.replace(pos, TalipotBitmapDirSym.size(), TalipotBitmapDir);
                            }

                            if (size_t pos = value.find(TulipBitmapDirSym);
                                pos != std::string::npos) {
                                value.replace(pos, TulipBitmapDirSym.size(), TalipotBitmapDir);
                            }
                ...
                if (size && canUsePubSetBuf) {
                    ...
                    while (numValues) {
                        ...
                        for (uint i = 0; i < valuesToRead; ++i) {
                            ...
                            if (pnViewProp) {
                                ...
                                if (size_t pos = value.find(TalipotBitmapDirSym);
                                    pos != std::string::npos) {
                                    value.replace(pos, TalipotBitmapDirSym.size(),
                                                  TalipotBitmapDir);
                                }
                    ...
                    for (uint i = 0; i < numValues; ++i) {
                        ...
                        // read edge id
                        if (!bool(inputData.is->read(reinterpret_cast<char *>(&e.id),
                                                     sizeof(uint)))) {
                            return false;
                        }
