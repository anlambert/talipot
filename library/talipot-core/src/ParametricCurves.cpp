...
void computeBezierPoints(const vector<Coord> &controlPoints, vector<Coord> &curvePoints,
    ...
    case 3:
        computeQuadraticBezierPoints(controlPoints[0], controlPoints[1], controlPoints[2],
                                     curvePoints, nbCurvePoints);
        break;

    case 4:
        computeCubicBezierPoints(controlPoints[0], controlPoints[1], controlPoints[2],
                                 controlPoints[3], curvePoints, nbCurvePoints);
        break;
...
static Coord computeCatmullRomPointImpl(const vector<Coord> &controlPoints, const float t,
    ...
    if (i == 0) {
        ...
        computeBezierSegmentControlPoints(controlPoints[i - 1], controlPoints[i],
                                          controlPoints[i + 1], lastPoint, bezierControlPoints,
                                          alpha);
        ...
        computeBezierSegmentControlPoints(controlPoints[i - 2], controlPoints[i - 1],
                                          controlPoints[i], lastPoint, bezierControlPoints, alpha);
    } else {
        computeBezierSegmentControlPoints(controlPoints[i - 1], controlPoints[i],
...
Coord computeOpenUniformBsplinePoint(const vector<Coord> &controlPoints, const float t,
    ...
    if (t == 0.0) {
        ...
        for (int i = 1; i <= int(curveDegree); ++i) {
            ...
            coeffs[curveDegree] =
                ((t - knotVal) / (clamp(knotVal + i * stepKnots, 0.0, 1.0) - knotVal)) *
                coeffs[curveDegree];
        }
...
void computeOpenUniformBsplinePoints(const vector<Coord> &controlPoints, vector<Coord> &curvePoints,
    ...
    curvePoints.resize(nbCurvePoints);
    TLP_PARALLEL_MAP_INDICES(nbCurvePoints, [&](uint i) {
        curvePoints[i] = computeOpenUniformBsplinePoint(controlPoints, i / float(nbCurvePoints - 1),
                                                        curveDegree);
    });
}
}
