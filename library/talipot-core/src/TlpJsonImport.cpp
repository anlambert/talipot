...
class TlpJsonGraphParser : public YajlParseFacade {
  public:
    ...
    void parseEndArray() override {
        ...
        // if the current array was not an edge but was the array of edges, we are done parsing
        // edges
    ...
    void parseString(const std::string &value) override {
            ...
            if (_currentProperty) {
                if (_parsingPropertyDefaultNodeValue) {
                    if (_parsingPathViewProperty) {
                        ...
                        } else if (size_t pos2 = value.find(TulipBitmapDirSym);
                                   pos2 != string::npos) {
                            string dValue(value);
                ...
                if (_parsingPropertyNodeValues) {
                    // if parsing a graphproperty, add it in the temporary buffer so the values are
                ...
                if (_parsingPropertyEdgeValues) {
                    ...
                    if (_parsingPathViewProperty) {
                        // if needed replace symbolic path by real path
                        string eValue(value);
                        if (size_t pos = eValue.find(TalipotBitmapDirSym);
                            pos != std::string::npos) {
                            eValue.replace(pos, TalipotBitmapDirSym.size(), TalipotBitmapDir);
                        }
                        if (size_t pos2 = eValue.find(TulipBitmapDirSym);
                            pos2 != std::string::npos) {
        ...
        if (_parsingAttributes) {
            ...
            if (_currentAttributeTypeName.empty()) {
                ...
                bool result =
                    _dataSet->readData(data, _currentAttributeName, _currentAttributeTypeName);
