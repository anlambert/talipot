...
void GraphUpdatesRecorder::removeFromIncidence(flat_hash_map<node, std::vector<edge>> &incidences,
    ...
    if (const auto itIncidence = incidences.find(n); itIncidence != incidences.end()) {
        ...
        if (const auto &it = std::find(incidence.begin(), incidence.end(), e);
            it != incidence.end()) {
            incidence.erase(it);
        }
    }
...
void GraphUpdatesRecorder::recordNewNodeValues(PropertyInterface *p) {
    ...
    if (oldNodeDefaultValues.contains(p)) {
        ...
        if (const auto itp = oldValues.find(p);
            itp != oldValues.end() && itp->second.recordedNodes) {
...
void GraphUpdatesRecorder::recordNewEdgeValues(PropertyInterface *p) {
    ...
    if (oldEdgeDefaultValues.contains(p)) {
        ...
        if (const auto itp = oldValues.find(p);
            itp != oldValues.end() && itp->second.recordedEdges) {
    ...
    for (const auto &[g, edges] : edgesToAdd) {
        ...
        for (edge e : edges) {
            if (const auto itEnds = edgesEnds.find(e); itEnds != edgesEnds.end()) {
                ...
                // restoration of an edge in a subgraph that was already an element of the root
                // graph (i.e., not a newly added edge)
