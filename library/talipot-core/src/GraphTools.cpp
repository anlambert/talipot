        ...
        while (!fifo.empty()) {
            ...
            for (auto adjit : graph->getOutEdges(n1)) {
                ...
                if (pluginProgress) {
                    ...
                    if (edgeCount == 200) {
                        if (pluginProgress->progress(nbSelectedNodes * 100 / graph->numberOfNodes(),
                                                     100) != ProgressState::TLP_CONTINUE) {
                            return;
                        }
...
static void bfs(const Graph *graph, node root, NodeVectorProperty<bool> &visited,
    ...
    while (!queue.empty()) {
        ...
        for (auto e : getIncidentEdgesIterator(
                 graph, current, directed ? EdgeType::DIRECTED : EdgeType::UNDIRECTED)) {
...
unsigned makeSelectionGraph(const Graph *graph, BooleanProperty *selection, bool *test) {
    ...
    for (auto e : selection->getEdgesEqualTo(true, graph)) {
        ...
        if (!selection->getNodeValue(src)) {
            tlp::debug() << "[Make selection a graph] node #" << src.id << " source of edge #"
                         << e.id << " automatically added to selection." << std::endl;
