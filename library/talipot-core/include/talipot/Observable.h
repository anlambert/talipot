...
#ifndef TALIPOT_OBSERVABLE_H
#define TALIPOT_OBSERVABLE_H
...
/**
 * @ingroup Observation
 * @brief The Observable class is the base of Talipot's observation system.
 *
 * Each class that wishes to send or receive notifications needs to inherit from Observable.
 *
 * Talipot has two separate mechanisms of observation, Observers and Listeners.
...
class TLP_SCOPE Observable {
  ...
  public:
     ...
     * Observable::unholdObservers(), and in this case the events will only be sent when there have
     * been as many calls to  Observable::unholdObservers() as to  Observable::holdObservers().
     *
    ...
    /**
     * @brief observersHoldCounter gives the number of times holdObservers() has been called without
     * a matching unholdObservers() call.
  ...
  public:
     ...
     * It is highly recommended to check if there are observers/listeners to send the event to
     * before actually sending it to avoid the overhead of creating too many objects.
     *
     ...
     * If events were held, this vector can be pretty large, and if events were not held it is
     * likely it only contains a single element.
     *
  ...
  protected:
     ...
     *
     * The observation system automatically sends the DELETE event when the Observable is deleted,
     * but in the case you need to access members of the class inheriting from Observable, you need
     * the event sent *before* the outermost objects are destroyed.
     *
     * To achieve this, you can call this function in your destructor, and the DELETE event will be
     * sent.
     * This will allow your Listeners/Observers to access your members one last time before
     * deletion.
     *
     * @warning This function must be called only once per object.
     * Make sure no other class in the inheritance tree calls this function before adding this call
     * to your destructor.
     */
  ...
  private:
     ...
     * @return an iterator on 'in' objects (Observable), the iterator guarantees that all objects
     ...
     *
     * In case of nested unholding (highly unlikely), calling that function inside a hold/unhold
     * block could make the Observer not receive an event that was sent when it was connected.
     *
