...
//
  ...
  tlp_hash_map<node, std::vector<edge>> oldIncidences;
  // one 'set' for new incidences
  tlp_hash_map<node, std::vector<edge>> newIncidences;
  ...
  void recordIncidence(tlp_hash_map<node, std::vector<edge>> &, GraphImpl *, node, edge e = edge());
  void recordIncidence(tlp_hash_map<node, std::vector<edge>> &, GraphImpl *, node,
                       const std::vector<edge> &, uint);
  // remove an edge from a node's edges container
  void removeFromIncidence(tlp_hash_map<node, std::vector<edge>> &, edge, node);
