...
#ifndef TALIPOT_QUAD_TREE_H
#define TALIPOT_QUAD_TREE_H
...
class QuadTreeNode {
  ...
  public:
     ...
     * to the size of the box (equivalent to have several item at the same position on the screen)
                ...
                if (!find) {
                    for (auto *child : children) {
                        if (child != nullptr && child->_box.intersect(box)) {
                            ...
                            // branch of the tree thus we do not have to explore the other branches.
                            child->getElementsWithRatio(box, result, ratio);
                            break;
                        }
                    }
