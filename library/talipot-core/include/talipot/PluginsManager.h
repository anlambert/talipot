...
class TLP_SCOPE PluginsManager : public Observable, public Singleton<PluginsManager> {
  ...
  public:
     ...
     *
     * @param loader If there are errors, the loader is informed about them so they can be
     *displayed.
     * @return void
     **/
     ...
     * This method checks the plugin "pluginName" is currently loaded into Talipot and if it's of
     * type PluginType.
     ...
     * @return :list< tlp::Dependency, std::allocator< tlp::Dependency > > The list of dependencies
     *of the plug-in.
     **/
  ...
  private:
    ...
    bool pluginExistsImpl(const std::string &pluginName) {
        ...
        return (it != _plugins.end() &&
                (dynamic_cast<const PluginType *>(it->second.info) != nullptr));
    }

    template <typename PluginType>
    PluginType *getPluginObjectImpl(const std::string &name,
                                    tlp::PluginContext *context = nullptr) {
...
class TLP_SCOPE PluginEvent : public Event {
  public:
    ...
    PluginEvent(PluginEventType pluginEvtType, const std::string &pluginName)
        : Event(tlp::PluginsManager::instance(), EventType::TLP_MODIFICATION),
