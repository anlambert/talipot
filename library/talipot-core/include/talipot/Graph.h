...
class TLP_SCOPE Graph : public Observable {
  ...
  public:
     ...
     * @param progress A PluginProgress to report the progress of the operation, as well as the
     *final state. Defaults to nullptr.
     ...
     * @return :Graph* The newly created clone subgraph. nullptr will be returned if addSibling is
     *set to true and this graph is a root graph.
     **/
     ...
     * @param nodes The nodes to add to the subgraph. All the edges between these nodes are added
     * too.
     ...
     * Every edge and its source and target node contained in the selection is added to the
     * subgraph. Every edge connecting any two nodes in the resulting set of nodes is also added.
    ...
    /**
     * @brief Adds an existing node in the graph. This node is also added in all the ancestor
     * graphs. This node must exists in the graph hierarchy (which means it must exist in the root
     * graph). You cannot add a node to the root graph this way (as it must already be an element of
     * the root graph).
     ...
     * as with addNode(const tlp::node), the nodes must exist in the graph hierarchy and thus exist
     in the root graph,
    ...
    */
     ...
     * @warning The graph takes ownership of the Iterator.
     * @param itE
     * @param deleteInAllGraphs  Whether to delete in all its parent graphs or only in this graph.
     * By default only removes in the current graph.
     */
     ...
     *
     * @return tlp::node The first encountered node with input degree of 0, or an invalid node if
     *none was found.
     **/
     ...
     * This forwards the call to the template version of getLocalProperty(), with the correct
     * template parameter deduced from the propertyType parameter.
     *
     * The graph keeps ownership of the property, if you wish to remove it from the graph use
     * Graph::delLocalProperty().
     *
    ...
    /**
     * @brief Gets an iterator over the names of the properties inherited from this graph's
     ...
     * @param progress A PluginProgress to report the progress of the operation, as well as the
     * final state. Defaults to nullptr. some additional information.
    ...
    // updates management
    /**
     * @brief Saves the current state of the whole graph hierarchy and allows to revert to this
     * state later on, using pop(). All modifications except those altering the ordering of the
     * edges will be undone.
     *
     ...
     * //Also we always want to keep the value of our super property, so we pass it in the
     * collection of properties to leave unaffected by the pop(). graph->push(false,
     * propertiesToKeep);
     *
     ...
     * @param propertiesToPreserveOnPop A collection of properties whose state to preserve when
     * using pop().
     * @see pop()
     * @see popIfNoUpdates()
     * @see unpop()
     ...
     * Every edges from any node in the vector to another node of the graph will be replaced with
     * meta edges from the meta node to the other nodes.
