...
class TLP_SCOPE PropertyInterface : public Observable {
  ...
  public:
     ...
     * @param ifNotDefault If true, the copy will only be performed if the source node's value is
     ...
     * @param ifNotDefault If true, the copy will only be performed if the source edge's value is
     * not the default value.
     * @return True if the copy was performed, false otherwise.
     */
     ...
     * the values of the current property, because no event is sent for nodes/edges whose value is
     * the default value of the source property.
     */
     ...
     *
     * @return Whether the given string was a correct representation for this property's type. If
     * not, the default value is not set.
     */
    virtual bool setNodeDefaultStringValue(const std::string &value) = 0;

    /**
     * @brief Sets all the nodes value to the value represented by the string.
     *
     * For some types, some parsing will be necessary (e.g. LayoutProperty).
     * All previous values are lost and the represented value is assigned as the
     ...
     * property, no node value will be modified in it.
     *
    ...
    /**
     * @brief Sets all the edges value to the value represented by the string.
     *
     * For some types, some parsing will be necessary (e.g. LayoutProperty).
     ...
     * When given a Graph as parameter, only the number of nodes with a non default value belonging
     * to this graph is returned.
...
class TLP_SCOPE VectorPropertyInterface : public PropertyInterface {
  public:
     ...
     * @param openChar an optional character opening the list of elements. Default value is '(';
     * when set to '\0' it indicates that there is no opening character.
     * @param sepChar an optional character separating the elements of the list. Default value is
     * ','.
     * @param closeChar an optional character closing the list of elements. Default value is ')';
     * when set to '\0' it indicates that there is no opening character.
    ...
    virtual bool tokenize(const std::string &str, std::vector<std::string> &vect,
                          char openChar = '(', char sepChar = ',', char closeChar = ')') = 0;

    /**
     * @brief Sets a new vector represented by the vector of string parameter as the node value.
     * @param n The node on which to set the new value.
     * @param values A vector of strings listing the string representations of elements of the
     * vector to set on the node.
