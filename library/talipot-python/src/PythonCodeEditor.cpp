...
FindReplaceDialog::FindReplaceDialog(QPlainTextEdit *editor)
    ...
    connect(_ui->replaceAllButton, &QAbstractButton::clicked, this,
            &FindReplaceDialog::doReplaceAll);
...
void PythonCodeEditor::paintEvent(QPaintEvent *event) {
    ...
    if (isTooltipActive()) {
        ...
        for (int i = 0; i < toolTipLines.size(); ++i) {
            ...
            for (auto c : toolTipLines[i]) {
                w += charWidth(c.toLatin1());
            }
    ...
    if (data) {
        ...
        for (int i = 0; i < info.size(); ++i) {
            ...
            if (pinfo.position == curPos - 1) {
                ...
                if (pinfo.character == ')') {
                    match = matchRightParenthesis(textCursor().block(), std::make_pair('(', ')'), i,
                                                  -1);
            ...
            if (pinfo.position == curPos) {
                ...
                if (pinfo.character == '(') {
                    match =
                        matchLeftParenthesis(textCursor().block(), std::make_pair('(', ')'), i + 1);
                } else if (pinfo.character == '[') {
                    match =
                        matchLeftParenthesis(textCursor().block(), std::make_pair('[', ']'), i + 1);
                } else if (pinfo.character == '{') {
    ...
    if (commentShortcutsActivated() && e->modifiers() == modifier && e->key() == Qt::Key_D) {
        ...
        if (e->text() == "(" && textAfterCursor.trimmed().isEmpty()) {
            ...
            if (!type.isEmpty()) {
                ...
                showTooltip(textCursor().blockNumber(), textBeforeCursor.indexOf(funcName),
                            toolTipTxt);
            }
        } else if (e->text() == ")") {
            hideTooltip();
