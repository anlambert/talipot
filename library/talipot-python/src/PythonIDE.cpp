...
PythonIDE::PythonIDE(QWidget *parent)
    ...
    _ui->decreaseFontSizeButton->setIcon(
        FontIcon::icon(MaterialDesignIcons::MinusCircle, Qt::black));
    _ui->increaseFontSizeButton->setIcon(
        FontIcon::icon(MaterialDesignIcons::PlusCircle, Qt::black));
    ...
    connect(_ui->newStringModuleButton, &QAbstractButton::clicked, this,
            &PythonIDE::newStringModule);
    ...
    connect(_ui->savePluginAsButton, &QAbstractButton::clicked, this,
            &PythonIDE::savePythonPluginAs);
    connect(_ui->registerPluginButton, &QAbstractButton::clicked, this,
            &PythonIDE::registerPythonPlugin);
    connect(_ui->removePluginButton, &QAbstractButton::clicked, this,
            &PythonIDE::removePythonPlugin);
    ...
    connect(_ui->modulesTabWidget, &PythonEditorsTabWidget::fileSaved, this,
            &PythonIDE::moduleSaved);
    connect(_ui->pluginsTabWidget, &PythonEditorsTabWidget::fileSaved, this,
            &PythonIDE::pluginSaved);

    connect(_ui->runScriptButton, &QAbstractButton::clicked, this,
            &PythonIDE::executeCurrentScript);
    connect(_ui->pauseScriptButton, &QAbstractButton::clicked, this,
            &PythonIDE::pauseCurrentScript);
    ...
    connect(_ui->modulesTabWidget, &PythonEditorsTabWidget::filesReloaded,
            _ui->mainScriptsTabWidget, &PythonEditorsTabWidget::reloadCodeInEditorsIfNeeded);
    ...
    connect(_ui->mainScriptsTabWidget, &PythonEditorsTabWidget::filesReloaded,
            _ui->modulesTabWidget, &PythonEditorsTabWidget::reloadCodeInEditorsIfNeeded);
    connect(_ui->mainScriptsTabWidget, &PythonEditorsTabWidget::filesReloaded,
            _ui->pluginsTabWidget, &PythonEditorsTabWidget::reloadCodeInEditorsIfNeeded);
    connect(_ui->pluginsTabWidget, &PythonEditorsTabWidget::filesReloaded, _ui->modulesTabWidget,
            &PythonEditorsTabWidget::reloadCodeInEditorsIfNeeded);
    connect(_ui->pluginsTabWidget, &PythonEditorsTabWidget::filesReloaded,
            _ui->mainScriptsTabWidget, &PythonEditorsTabWidget::reloadCodeInEditorsIfNeeded);
    ...
    APIDataBase::instance().loadApiFile(tlpStringToQString(tlp::TalipotShareDir) + "/api/Python-" +
                                        PythonInterpreter::instance().getPythonVersionStr() +
                                        ".api");
    ...
    _ui->pipLabel->setText(
        ...
        "manage external packages that can be directly imported in the current Python session "
        "after "
        "their installation.");
...
void PythonIDE::loadModule() {
    QString fileName =
        QFileDialog::getOpenFileName(this, "Open module", "", "Python script (*.py)");
    loadModule(fileName);
}
...
void PythonIDE::saveModule(int tabIdx, bool saveAs) {
    if (tabIdx >= 0 && tabIdx < _ui->modulesTabWidget->count()) {
        ...
        QString fileName;
        if (saveAs) {
            fileName = QFileDialog::getSaveFileName(this, tr("Set module filename"), "",
                                                    "Python script (*.py)");
            if (fileName.isEmpty()) {
                return;
            }
...
void PythonIDE::loadPythonPlugin() {
    QString fileName = QFileDialog::getOpenFileName(this, "Open Talipot Python plugin", "",
                                                    "Python script (*.py)");
    ...
    if (checkAndGetPluginInfoFromSrcCode(pluginCode, pluginName, pluginClassName, pluginType,
        ...
        if (pluginClassName.isEmpty() || pluginName.isEmpty()) {
            QMessageBox::critical(this, "Error",
                                  "Unable to retrieve the plugin class name and the plugin name "
...
void PythonIDE::savePythonPlugin(int tabIdx, bool saveAs) {
    ...
    if (tabIdx >= 0 && tabIdx < _ui->pluginsTabWidget->count()) {
        ...
        QString fileName;
        if (saveAs) {
            fileName = QFileDialog::getSaveFileName(this, tr("Set plugin filename"), "",
                                                    "Python script (*.py)");
            if (fileName.isEmpty()) {
                return;
            }
                ...
                if (_project->exists(projectFile)) {
                    ...
                    if (fileName.startsWith("[no file]")) {
                        ...
                        _ui->mainScriptsTabWidget->setTabToolTip(
                            editorId,
                            "string main script, do not forget to save the current Talipot project "
    ...
    for (int i = 0; i < _ui->mainScriptsTabWidget->count(); ++i) {
        ...
        if (deleted == -1 || i != deleted) {
            ...
            if (fileName.isEmpty()) {
                ...
                // only save a relative path for reloading the code from the file even on a
                // different computer this is useful for instance when Talipot project and Python
                // files are stored on a git repository
...
void PythonIDE::executeCurrentScript() {
    ...
    _scriptRunning = true;
    bool scriptExecOk =
        _pythonInterpreter->runGraphScript("__main__", "main", graph, scriptFileName);
    ...
    if (curTabText[curTabText.size() - 1] == '*' || fileName.isEmpty() ||
        ...
        if (button == QMessageBox::Save) {
            if (fileName.isEmpty()) {
                ...
                QString saveFileName = QFileDialog::getSaveFileName(this, tr("Save Python file"),
                                                                    "", "Python file (*.py)");
        ...
        QMessageBox::StandardButton button =
            QMessageBox::question(QApplication::activeWindow(), QString("Close Python code editor"),
                                  "Are you sure you want to close that code editor ?",
...
Graph *PythonIDE::getSelectedGraph() const {
    return _graphsModel->data(_ui->graphComboBox->selectedIndex(), Model::GraphRole)
        .value<Graph *>();
}
