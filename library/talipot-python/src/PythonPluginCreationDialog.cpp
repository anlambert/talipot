...
void PythonPluginCreationDialog::accept() {
    ...
    if (moduleName.contains(" ")) {
        QMessageBox::critical(this, "Error",
                              "The Python module name cannot contain any whitespace.");
        return;
    }
    ...
    while (PythonInterpreter::pythonReservedCharacters[i]) {
        if (moduleName.contains(PythonInterpreter::pythonReservedCharacters[i++])) {
            QMessageBox::critical(this, "Error",
                                  "The Python module name contains an invalid character.");
            return;
        }
    }
    ...
    if (_ui->pluginClassName->text().contains(" ")) {
        QMessageBox::critical(this, "Error",
                              "The Python class name cannot contain any whitespace.");
        return;
    }
    ...
    while (PythonInterpreter::pythonReservedCharacters[i]) {
        if (_ui->pluginClassName->text().contains(
                PythonInterpreter::pythonReservedCharacters[i++])) {
            QMessageBox::critical(this, "Error",
                                  "The Python class name contains an invalid character.");
            return;
        }
    }
...
void PythonPluginCreationDialog::selectPluginSourceFile() {
    QString fileName = QFileDialog::getSaveFileName(this, tr("Set Plugin source file"), "",
                                                    "Python script (*.py)");
