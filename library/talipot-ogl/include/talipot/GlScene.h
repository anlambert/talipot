...
class TLP_GL_SCOPE GlScene : public Observable {
  ...
  public:
     ...
     * @param calculator By default GlScene use a GlCPULODCalculator to compute LOD but you can
     * change this default lod calculator, to do that : put your calculator in constructor
     * parameters Available calculators are : GlCPULODCalculator and GlQuadTreeLODCalculator
     */
     ...
     * This function is the most important function of GlScene. If you want to render a scene into
     * an OpenGL widget : call this function
     */
    void draw();
     ...
     * @param xWhiteFactor the white part on x borders (left and right), the computed empty space
     * size will be stored in (if xWhiteFactor != nullptr)
     * @param yWhiteFactor the white part on y borders (top and bottom), the computed empty space
     * size will be stored in (if yWhiteFactor != nullptr)
