...
#ifndef TALIPOT_GL_SHADER_PROGRAM_H
#define TALIPOT_GL_SHADER_PROGRAM_H
...
class TLP_GL_SCOPE GlShader {
  ...
  public:
    ...
    /**
     * Method only relevant for geometry shaders. Set the graphic primitive type this geometry
     * shader takes as input. Note that when modifying the input primitive type, the associated
     * shader program (whose object is from type GlShaderProgram) has to be relinked for changes to
     * take effect.
     *
     * \param inputPrimitiveType the type of graphic primitive the geometry shader takes as input.
     *        (must be one from the following list : GL_POINTS, GL_LINES, GL_LINES_ADJACENCY_EXT,
     ...
     * shader will output.
     * Note that when modifying the output primitive type, the associated shader program (whose
     * object is from type GlShaderProgram) has to be relinked for changes to take effect.
     *
     * \param outputPrimitiveType the type of graphics primitives the geometry shader will output
     *        (must be one of the following list : GL_POINTS, GL_LINE_STRIP, GL_TRIANGLE_STRIP)
     */
...
class TLP_GL_SCOPE GlShaderProgram {
  ...
  public:
    ...
    /**
     * A static function which returns true if geometry shaders are supported by the host graphic
     * card
     */
     ...
     *
     * \param shaderType the type of the shader object to add (must be Vertex or Fragment)
     * \param shaderSrcFilename the absolute path to the file containing the shader object source
     * code
     */
    ...
    /**
     * Activate the shader program. If the shader program has not been linked, the link method will
     * be called.
     */
    void activate();
    ...
    void setUniformVec3Int(const std::string &variableName, const int i1, const int i2,
                           const int i3);
    void setUniformVec4Int(const std::string &variableName, const Vec4i &vec4i);
    void setUniformVec4Int(const std::string &variableName, const int i1, const int i2,
    ...
    void setUniformFloatArray(const std::string &variableName, const uint fCount, const float *f);

    template <uint SIZE>
    void setUniformVec2FloatArray(const std::string &variableName,
                                  const Array<Vector<float, 2>, SIZE> &vecvec2f);
    void setUniformVec2FloatArray(const std::string &variableName, const uint vec2fCount,
                                  const float *f);

    template <uint SIZE>
    void setUniformVec3FloatArray(const std::string &variableName,
                                  const Array<Vec3f, SIZE> &vecvec3f);
    void setUniformVec3FloatArray(const std::string &variableName, const uint vec3fCount,
                                  const float *f);

    template <uint SIZE>
    void setUniformVec4FloatArray(const std::string &variableName,
                                  const Array<Vec4f, SIZE> &vecvec4f);
    void setUniformVec4FloatArray(const std::string &variableName, const uint vec4fCount,
                                  const float *f);
    ...
    void setUniformMat2FloatArray(const std::string &variableName,
                                  ...
                                  const bool transpose = false);
    void setUniformMat2FloatArray(const std::string &variableName, const uint mat2fCount,
    ...
    void setUniformMat3FloatArray(const std::string &variableName,
                                  ...
                                  const bool transpose = false);
    void setUniformMat3FloatArray(const std::string &variableName, const uint mat3fCount,
                                  const float *f, const bool transpose = false);
    ...
    void setUniformMat4FloatArray(const std::string &variableName,
                                  ...
                                  const bool transpose = false);
    void setUniformMat4FloatArray(const std::string &variableName, const uint mat4fCount,
                                  const float *f, const bool transpose = false);

    template <uint SIZE>
    void setUniformIntArray(const std::string &variableName, const Vector<int, SIZE> &veci);
    void setUniformIntArray(const std::string &variableName, const uint iCount, const int *i);

    template <uint SIZE>
    void setUniformVec2IntArray(const std::string &variableName,
                                const Array<Vec2i, SIZE> &vecvec2i);
    void setUniformVec2IntArray(const std::string &variableName, const uint vec2iCount,
                                const int *i);

    template <uint SIZE>
    void setUniformVec3IntArray(const std::string &variableName,
                                const Array<Vector<int, 3>, SIZE> &vecvec3i);
    void setUniformVec3IntArray(const std::string &variableName, const uint vec3iCount,
                                const int *i);

    template <uint SIZE>
    void setUniformVec4IntArray(const std::string &variableName,
                                const Array<Vec4i, SIZE> &vecvec4i);
    void setUniformVec4IntArray(const std::string &variableName, const uint vec4iCount,
                                const int *i);

    template <uint SIZE>
    void setUniformBoolArray(const std::string &variableName, const Array<bool, SIZE> &vecb);
    void setUniformBoolArray(const std::string &variableName, const uint bCount, const bool *b);

    template <uint SIZE>
    void setUniformVec2BoolArray(const std::string &variableName,
                                 const Array<Array<bool, 2>, SIZE> &vecvec2b);
    void setUniformVec2BoolArray(const std::string &variableName, const uint vec2bCount,
                                 const bool *b);

    template <uint SIZE>
    void setUniformVec3BoolArray(const std::string &variableName,
                                 const Array<Array<bool, 3>, SIZE> &vecvec3b);
    void setUniformVec3BoolArray(const std::string &variableName, const uint vec3bCount,
                                 const bool *b);

    template <uint SIZE>
    void setUniformVec4BoolArray(const std::string &variableName,
                                 const Array<Array<bool, 4>, SIZE> &vecvec4b);
    void setUniformVec4BoolArray(const std::string &variableName, const uint vec4bCount,
                                 const bool *b);
