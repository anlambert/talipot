...
class TLP_GL_SCOPE GlQuantitativeAxis : public GlAxis {
  ...
  public:
     ...
     * \ascendingOrder If true, the min value will be at the bottom end and the max will be at the
     * top end if the axis is vertical (min at the left and max at the right if it is horizontal).
     * If false this positions are switched
     */
     ...
     * \param drawFirstLabel If false, the first graduation label will not be drawn (useful when
     * some axis have the same base coord to avoid labels overlapping)
     */
    ...
    void setAxisParameters(const int min, const int max, const uint incrementStep,
        ...
        setAxisParameters(static_cast<long long>(min), static_cast<long long>(max),
                          static_cast<ullong>(incrementStep), axisGradsLabelsPosition,
                          drawFirstLabel);
    }
    ...
    /**
     * Method to update the axis drawing. It has to be called when one (or more) of the setters
     * method above has been used. This method redraw the whole axis and the graduations.
     */
