...
void buildCurvePoints(const vector<Coord> &vertices, const vector<float> &sizes,
    ...
    if (startN != vertices[0]) {
        ...
        inversion =
            computeExtrusion(vertices[0] - (vertices[1] - vertices[0]), vertices[0], vertices[1],
                             sizes[0], inversion, result, false, twoPointsCurve);
    }

    for (uint i = 1; i < sz - 1; ++i) {
        inversion = computeExtrusion(vertices[i - 1], vertices[i], vertices[i + 1], sizes[i],
                                     inversion, result, false, twoPointsCurve);
    }

    if (endN != vertices[sz - 1]) {
        inversion =
            computeExtrusion(vertices[sz - 2], vertices[sz - 1], endN, sizes[sizes.size() - 1],
