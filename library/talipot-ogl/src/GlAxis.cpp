    ...
    for (uint i = 0; i < axisGradsLabels.size(); ++i) {
        ...
        if (axisOrientation == HORIZONTAL_AXIS) {
            ...
            if (axisGradsPosition == LEFT_OR_BELOW) {
                graduationLabel->setPosition(Coord(axisBaseCoord.getX() +
                                                       i * spaceBetweenAxisGrads -
...
void GlAxis::buildAxisLine() {
    ...
    if (axisOrientation == VERTICAL_AXIS) {
        for (uint i = 1; i <= nbLineSegments; ++i) {
            axisLine->addPoint(Coord(axisBaseCoord.getX(), axisBaseCoord.getY() + i * step),
                               axisColor);
        }
    } else if (axisOrientation == HORIZONTAL_AXIS) {
        for (uint i = 1; i <= nbLineSegments; ++i) {
            axisLine->addPoint(Coord(axisBaseCoord.getX() + i * step, axisBaseCoord.getY()),
                               axisColor);
        }
    }
...
Coord GlAxis::computeCaptionCenter(const bool captionFrame) {
    ...
    if (axisOrientation == VERTICAL_AXIS) {
        if (captionPosition == RIGHT) {
            ...
            captionCenter = Coord(axisBaseCoord.getX(), axisBaseCoord.getY() + axisLength +
                                                            captionOffset + captionHeight / 2);
        }
    } else if (axisOrientation == HORIZONTAL_AXIS) {
        if (captionPosition == RIGHT) {
            captionCenter =
                Coord(axisBaseCoord.getX() + axisLength + captionOffset + captionWidth / 2,
                      axisBaseCoord.getY());
        } else if (captionPosition == LEFT) {
            captionCenter = Coord(axisBaseCoord.getX() - captionOffset - captionWidth / 2,
                                  axisBaseCoord.getY());
        } else if (captionPosition == BELOW) {
