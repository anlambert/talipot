...
void GlNode::draw(float lod, const GlGraphInputData *data, Camera *camera) {
    ...
    // (because if we do a scale of (x,y,0) and if we have a normal like (0,0,1) the new normal
    // after scale will be (0,0,0) and we will have light problem)
    Size nodeSize = size;
    if (nodeSize[2] == 0) {
    ...
    if (data->glGlyphRenderer()->renderingHasStarted() && glyphObj->shaderSupported()) {
        ...
        if (selected) {
            glStencilFunc(GL_LEQUAL, data->renderingParameters()->getSelectedNodesStencil(),
                          0xFFFF);
        } else {
