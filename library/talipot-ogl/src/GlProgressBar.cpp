...
GlProgressBar::GlProgressBar(const Coord &centerPosition, const uint width, const uint height,
...
void GlProgressBar::progress_handler(int step, int max_step) {
    ...
    auto *percentLabel =
        new GlLabel(Coord(progressBarTLCorner.getX() + (progressBarMaxWidth / 2.),
                          progressBarTLCorner.getY() - (progressBarHeight / 2.), 0),
                    Size(((1. / 10.) * progressBarMaxWidth), ((8. / 10.) * progressBarHeight), 0),
                    commentColor);
    stringstream str;
    str << currentPercent << " %";
