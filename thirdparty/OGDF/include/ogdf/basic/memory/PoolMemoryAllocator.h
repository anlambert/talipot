 ...
 * \c OGDF_POINTER_SIZE bytes are allocated in addition to store a pointer to another segment of
 * memory.
 *
...
public:
  ...
  //! Flushes all free but allocated bytes (#s_tp) to the thread-global list (#s_pool) of allocated
  //! bytes.
...
private:
  ...
  //! Holds all allocated memory independently of whether it is cleared in chunks of size
  //! #BLOCK_SIZE.
