...
void CubeOutLinedTransparent::draw(node n, float lod) {
  GlBox::draw(Color(0, 0, 0, 0), (*glGraphInputData->borderColors())[n],
              (*glGraphInputData->borderWidths())[n], "", lod);
}
...
public:
  ...
  void draw(edge e, node, const Color &fillColor, const Color &borderColor, float lod) override {
    ...
    GlBox::draw(fillColor, borderColor, (*edgeExtGlGraphInputData->borderWidths())[e], textureName,
                lod);
