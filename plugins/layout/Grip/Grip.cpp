    ...
    for (uint i = 0; i <= end; ++i) {
        ...
        for (uint j = 0; j < neighbors[n].size(); ++j) {
            cerr << "distance euclidienne "
                 << (result->getNodeValue(n) - result->getNodeValue(neighbors[n][j])).norm() /
                        edgeLength
        ...
        for (uint i = start; i <= end; ++i) {
            ...
            for (uint j = 0; j < neighbors[currNode].size(); ++j) {
                ...
                if (!(euclidian_dist_sqr > 1E-4)) {
                    ...
                    c_tmp = Coord(alpha - (2. * alpha * randomNumber(1)),
                                  alpha - (2. * alpha * randomNumber(1)),
                                  alpha - (2. * alpha * randomNumber(1)));
...
void Grip::init() {
    ...
    for (auto n : currentGraph->nodes()) {
        Coord alea =
            Coord(diam - (2. * diam * randomNumber(1)), diam - (2. * diam * randomNumber(1)),
...
void Grip::set_nbr_size() {
    ...
    for (uint i = 1; i < misf->index.size(); ++i) {
        if (i >= maxLevel) {
            levelToNbNeighbors[i] = min(
                uint(sched(misf->index.size() - i, 0, 2, 10000, 1) * maxCxty / (misf->index[i])),
                uint(misf->index[i] - 1));
        } else {
