            ...
            if (component.size() == 3) {
                ...
                if (e.isValid()) {
                    ...
                    if (graph->source(e) == n) {
                        ...
                        bends.push_back(
                            Coord(2.f * spacing + c.getX() / 2.f + c2.getX() + c3.getX() / 2.f,
                                  float(max), 0));
                    } else {
                        bends.push_back(
                            Coord(2.f * spacing + c.getX() / 2.f + c2.getX() + c3.getX() / 2.f,
...
void MixedModel::computeCoords() {
    ...
    for (uint k = 1; k < size_V; ++k) { // parcours de chaque partition
        ...
        node cl = leftV(k),
             cr =
                 rightV(k); // recherche du left et right de Vk, qui est cherché à l'aide de EdgesIN
        ...
        if (EdgesIN[Z0].size() >= 3) {
            ...
            ftmp =
                ((out_r < (nodeSize.get(Z0.id)).getX() / 2.f) ? (nodeSize.get(Z0.id)).getX() / 2.f
                                                              : out_r) -
                dxr;
