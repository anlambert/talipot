...
class GMLExport : public tlp::ExportModule {
  public:
    PLUGININFORMATION(
        ...
        "gml-technical-report.pdf for details.</p>",
        "1.0", "File")
    ...
    bool exportGraph(ostream &os) override {
        ...
        for (auto n : graph->nodes()) {
            ...
            os << "fill \"#" << hex << setfill('0') << setw(2)
               << int(colors->getNodeValue(n).getR()) << hex << setfill('0') << setw(2)
               << int(colors->getNodeValue(n).getG()) << hex << setfill('0') << setw(2)
               << int(colors->getNodeValue(n).getB()) << "\"" << endl;
