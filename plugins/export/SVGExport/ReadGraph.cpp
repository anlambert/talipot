...
static bool treatEdges(Graph *graph, tlp::PluginProgress *pp, ExportInterface &exportint,
    ...
    for (auto e : graph->edges()) {
        ...
        if (edge_extremities) {
            src_anchor_shape_type = static_cast<EdgeExtremityShape::EdgeExtremityShapes>(
                srcanchorshape->getEdgeValue(e));
            tgt_anchor_shape_type = static_cast<EdgeExtremityShape::EdgeExtremityShapes>(
        ...
        if (src_anchor_shape_type != EdgeExtremityShape::None ||
            ...
            ret = exportint.exportEdgeExtremity(
                id_src_shape, id_tgt_shape, src_anchor_shape_type, tgt_anchor_shape_type,
                colors->getEdgeValue(e), id_src_grad, id_tgt_grad, iconName->getEdgeValue(e));
        ...
        if (!edge_color_interpolation) {
            ...
            ret = exportint.exportEdge(
                ...
                layout->getEdgeValue(e), colors->getNodeValue(src), colors->getNodeValue(tgt),
                width, src_anchor_shape_type, id_src_shape, tgt_anchor_shape_type, id_tgt_shape,
                edgeVertices);
        }
        ...
        if (edge_labels) {
            Coord c =
                edgeVertices[edgeVertices.size() / 2] + edgeVertices[edgeVertices.size() / 2 - 1];
            ret = exportint.addLabel("edge", label->getEdgeValue(e), labelcolor->getEdgeValue(e),
                                     c /= 2, fontsize->getEdgeValue(e), sizes->getEdgeValue(e));
...
bool ReadGraph::readGraph(Graph *graph, tlp::DataSet *ds, tlp::PluginProgress *pp,
    ...
    // Analysing edges
    ret =
        treatEdges(graph, pp, exportint, i, nb_elements, sizes, colors, layout, shape,
    ...
    while (!metanodeVertices.empty()) {
        ...
        for (auto metanode : metanodeVertices) {
            ...
            ret = treatEdges(metagraph, pp, exportint, i, nb_elements, sizes, colors, layout, shape,
                             srcanchorshape, tgtanchorshape, label, labelcolor,
                             edge_color_interpolation, edge_size_interpolation, edge_extremities,
            ...
            ret = treatNodes(metagraph, pp, exportint, i, nb_elements, sizes, colors, layout, shape,
                             rotation, borderwidth, label, labelcolor, bordercolor,
