...
class FileSystem : public tlp::ImportModule {
  public:
    ...
    FileSystem(tlp::PluginContext *context)
        : ImportModule(context), _absolutePaths(nullptr), _baseNames(nullptr),
          _createdDates(nullptr), _fileNames(nullptr), _isDir(nullptr), _isExecutable(nullptr),
          _isReadable(nullptr), _isSymlink(nullptr), _isWritable(nullptr),
          _lastModifiedDates(nullptr), _lastReadDates(nullptr), _owners(nullptr),
          _permissions(nullptr), _suffixes(nullptr), _sizes(nullptr), _fontIcon(nullptr),
    ...
    bool importGraph() override {
        ...
        if (!rootInfo.exists()) {
            if (pluginProgress) {
                pluginProgress->setError("Provided directory " +
  ...
  private:
    tlp::node addFileNode(const QFileInfo &info, tlp::Graph *g) {
        ...
        _lastModifiedDates->setNodeValue(n,
                                         tlp::QStringToTlpString(info.lastModified().toString()));
