...
Histogram::Histogram(Graph *graph, Graph *edgeGraph, flat_hash_map<edge, node> &edgeMap,
...
void Histogram::createAxis() {
    ...
    if (uniformQuantification) {
        ...
        if (integerScale) {
            ...
            if (axisMax != LLONG_MIN) {
                ...
                xAxis->setAxisParameters(axisMin, axisMax, incrementStep, GlAxis::LEFT_OR_BELOW,
                                         true);
            } else {
    ...
    if (cumulativeFreqHisto) {
        if (dataLocation == ElementType::NODE) {
            refSizeY = yAxis->getAxisPointCoordForValue(graph->numberOfNodes()).getY() /
                       graph->numberOfNodes();
        } else {
            refSizeY = yAxis->getAxisPointCoordForValue(graph->numberOfEdges()).getY() /
                       graph->numberOfEdges();
        }
    } else {
    ...
    for (uint i = 0; i < nbHistogramBins; ++i) {
        ...
        if (binSize > 0) {
            ...
            if (dataLocation == ElementType::NODE) {
                ...
                quadColor = Color(quadColorCumul[0], quadColorCumul[1], quadColorCumul[2],
                                  quadColorCumul[3]);
            } else {
