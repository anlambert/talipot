...
void GlGlyphScale::setGlyphsList(const vector<int> &glyphsList) {
    ...
    if (orientation == Vertical) {
        ...
        for (auto n : glyphGraph->nodes()) {
            glyphGraphLayout->setNodeValue(n,
                                           Coord(xCenter, baseCoord.getY() + i * size + size / 2));
            int oldI = i++;
...
void HistogramMetricMapping::initInteractor() {
    ...
    if (curve == nullptr || lastXAxisLength != histoXAxis->getAxisLength()) {
        Coord curveEndPoint = {histoXAxis->getAxisBaseCoord().getX() + histoXAxis->getAxisLength(),
                               histoYAxis->getAxisBaseCoord().getY() + histoYAxis->getAxisLength()};
    ...
    if (glColorScale == nullptr) {
        ...
        Coord newBaseCoord =
            curveStartPoint - Coord(histoYAxis->getMaxLabelWidth() + scaleWidth, 0);
    ...
    if (glSizeScale == nullptr) {
        glSizeScale = new GlSizeScale(
            sizeScaleConfigDialog->getMinSize(), sizeScaleConfigDialog->getMaxSize(),
            curveStartPoint - Coord(scaleAxisOffset, 0, 0), histoYAxis->getAxisLength(), scaleWidth,
            Color(0, 0, 0), GlSizeScale::Vertical);
    } else {
    ...
    if (glGlyphScale == nullptr) {
        glGlyphScale =
            new GlGlyphScale(curveStartPoint - Coord(histoYAxis->getMaxLabelWidth(), 0, 0),
        ...
        Coord newBaseCoord = curveStartPoint - Coord(histoYAxis->getMaxLabelWidth(), 0);
...
bool HistogramMetricMapping::eventFilter(QObject *widget, QEvent *e) {
    ...
    if (e->type() == QEvent::MouseMove) {
        ...
        if (!curveDragStarted) {
            Coord *anchor = curve->getCurveAnchorAtPointIfAny(
                glWidget->screenToViewport(
                    Coord(me->pos().x(), glWidget->height() - me->pos().y(), 0)),
        ...
        if (pointerUnderScale(sceneCoords)) {
            ...
            QAction *clickedAction = popupMenu->exec(cme->globalPos());
...
void HistogramMetricMapping::updateMapping(GlQuantitativeAxis *histoXAxis, uint nbHistogramBins) {
    ...
    if (mappingType == VIEWCOLOR_MAPPING || mappingType == VIEWBORDERCOLOR_MAPPING) {
        ...
        for (float x = histoXAxis->getAxisBaseCoord().getX();
             x <= histoXAxis->getAxisBaseCoord().getX() + histoXAxis->getAxisLength();
             x += increment) {
