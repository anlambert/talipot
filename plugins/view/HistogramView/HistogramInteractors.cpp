...
void HistogramInteractorMetricMapping::construct() {
    setConfigurationWidgetText(
        ...
        "picking the number of colors to use by using the spinbox located above the colors "
        "table. " +
        "To select the colors to use, double click on the cells of the colors table and a color "
        "picker dialog will appear. A preview of the built color scale is displayed at the right "
        "of "
        "the color table.<br />" +
        ...
        "alt=\"\"><br />" +
        "The second tab of the dialog allow to load a previously saved color scale and give also "
        "the "
        ...
        "Once the color scale to use has been selected or configured, press the \"Ok\" "
        "button.</p>" +
        ...
        "The middle part of the dialog aims to configure the minimum and maximum size to use for "
        "the "
        "mapping.<br />" +
        "The bottom part of the dialog allows to select on which dimensions the size mapping has "
        "to "
        ...
        "histogram. " +
        "Take the line perpendicular to the horizontal axis of the histogram (the metric axis) "
        "which "
        ...
        "<img src=\":/HistoColorMapping.png\" width=\"280\" height=\"260\" border=\"0\" "
        "alt=\"\"><br "
        "/>" +
        "</p>" + "</body></html>");
...
class HistogramMouseShowElementInfo : public MouseShowElementInfo {
  ...
  public:
    ...
    ~HistogramMouseShowElementInfo() override = default;
  ...
  protected:
    ...
    QAbstractItemModel *buildModel(ElementType elementType, uint elementId,
        ...
        if (hView->getDataLocation() == ElementType::EDGE) {
