...
void HistogramView::setState(const DataSet &dataSet) {
    ...
    if (!selectedProperties.empty()) {
        ...
        for (const auto &selectedProperty : selectedProperties) {
            ...
            if (histogramParametersMap[selectedProperty].get("nb histogram bins",
                                                             nbHistogramBins)) {
            ...
            if (histogramParametersMap[selectedProperty].get("x axis nb graduations",
                                                             nbXGraduations)) {
                histo->setLayoutUpdateNeeded();
            ...
            if (histogramParametersMap[selectedProperty].get("x axis custom scale",
                                                             useCustomAxisScale)) {
                histo->setLayoutUpdateNeeded();
                ...
                if (useCustomAxisScale) {
                    ...
                    histogramParametersMap[selectedProperty].get("x axis scale min",
                                                                 axisScale.first);
                    histogramParametersMap[selectedProperty].get("x axis scale max",
                                                                 axisScale.second);
                    histo->setXAxisScale(axisScale);
                }
            }

            if (histogramParametersMap[selectedProperty].get("y axis custom scale",
                                                             useCustomAxisScale)) {
                histo->setLayoutUpdateNeeded();
                ...
                if (useCustomAxisScale) {
                    std::pair<double, double> axisScale(0, 0);
                    histogramParametersMap[selectedProperty].get("y axis scale min",
                                                                 axisScale.first);
                    histogramParametersMap[selectedProperty].get("y axis scale max",
                                                                 axisScale.second);
                    histo->setXAxisScale(axisScale);
                }
            }
...
void HistogramView::buildHistograms() {
    ...
    for (size_t i = 0; i < selectedProperties.size(); ++i) {
        ...
        if (!histogramsMap.contains(selectedProperties[i])) {
            auto *histoOverview = new Histogram(
