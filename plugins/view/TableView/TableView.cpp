...
void TableView::graphChanged(tlp::Graph *g) {
    ...
    if (_model != nullptr) {
        for (int i = 0; i < _model->columnCount(); ++i) {
            ...
            bool checked =
                !visibleProperties.isEmpty() ? visibleProperties.contains(propName) : true;
    ...
    if (property == anyProperty) {
        for (int i = 0; i < _model->columnCount(); ++i) {
            if (!_ui->table->horizontalHeader()->isSectionHidden(i))
                props += _model->headerData(i, Qt::Horizontal, Model::PropertyRole)
                             .value<PropertyInterface *>();
        }
...
bool TableView::setAllHighlightedRows(PropertyInterface *prop) {
    ...
    QVariant val = ItemDelegate::showEditorDialog(
        NODES_DISPLAYED ? ElementType::NODE : ElementType::EDGE, prop, g,
...
void TableView::showCustomContextMenu(const QPoint &pos) {
    ...
    if (propIsInherited) {
        ...
        setAll->setToolTip("Choose a new " + eltsName +
    ...
    if (propName != "viewLabel") {
        subMenu = contextMenu.addMenu(
            FontIcon::icon(MaterialDesignIcons::OrderAlphabeticalAscending), "To label(s) of ");
        ...
        selectedToLabels->setToolTip(
            "Set the values of the current property as labels of the selected " + eltsName +
            OF_GRAPH);

        if (highlightedRows.size() > 1) {
            highlightedToLabels = subMenu->addAction("Rows highlighted " + eltsName);
            highlightedToLabels->setToolTip(
                "Set the values of the current property as labels of the " + eltsName +
                " displayed in the currently highlighted row(s)");
        } else {
    ...
    action->setEnabled(false);
    contextMenu.addSeparator();
    QAction *toggleAction = contextMenu.addAction(FontIcon::icon(MaterialDesignIcons::SelectionOff),
                                                  "Toggle selection");
...
void TableView::showHorizontalHeaderCustomContextMenu(const QPoint &pos) {
    ...
    if (!propertiesEditor->isReservedPropertyName(propName.c_str())) {
        renameProp =
            contextMenu.addAction(FontIcon::icon(MaterialDesignIcons::RenameBox), "Rename");
    ...
    QMenu *subMenu =
        contextMenu.addMenu(FontIcon::icon(MaterialDesignIcons::Pen), "Set values of ");
    ...
    if (propIsInherited) {
        ...
        edgesSetAll->setToolTip(
            "Choose a new edge default value to reset the values of all edges " + OF_PROPERTY);
    }
    ...
    nodesSelectedSetAll->setToolTip("Choose a value to be assigned to the selected nodes" +
                                    OF_GRAPH);
    QAction *edgesSelectedSetAll = subMenu->addAction("Selected edges" + OF_GRAPH);
    edgesSelectedSetAll->setToolTip("Choose a value to be assigned to the selected edges" +
                                    OF_GRAPH);
    ...
    if (propName != "viewLabel") {
        subMenu = contextMenu.addMenu(
            FontIcon::icon(MaterialDesignIcons::OrderAlphabeticalAscending), "To labels of ");
