...
AxisSlider::AxisSlider(const sliderType type, const Coord &sliderCoord, const float halfWidth,
    ...
    sliderLabel =
        new GlLabel(Coord(sliderCoord.getX(), sliderCoord.getY() + n * 1.75f * halfHeight),
        ...
        if (!axisSliderDragStarted && !slidersRangeDragStarted) {
            ...
            if ((axisSlidersMap[selectedAxis][TOP_SLIDER]->getSliderCoord().getY() + dy) >
                ...
                selectedAxis->setBottomSliderCoord(
                    ...
                    Coord(0.0f, dy));
                selectedAxis->setTopSliderCoord(
                    axisSlidersMap[selectedAxis][TOP_SLIDER]->getSliderCoord() + Coord(0.0f, dy));
            }
...
void ParallelCoordsAxisSliders::buildGlSliders(vector<ParallelAxis *> axis) {
    for (auto *ax : axis) {
        ...
        auto *axisBottomSlider = new AxisSlider(
            BOTTOM_SLIDER, ax->getBottomSliderCoord(), 2.5f * sliderMetricRef,
            2.0f * sliderMetricRef, redColor, ax->getAxisColor(), ax->getRotationAngle());
