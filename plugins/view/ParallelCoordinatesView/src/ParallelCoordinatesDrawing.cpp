...
void ParallelCoordinatesDrawing::createAxis(GlWidget *glWidget, GlProgressBar *progressBar) {
    ...
    for (const string &selectedProp : selectedProperties) {
        ...
        if (layoutType == PARALLEL) {
            if (nbAxis != 1) {
                coord =
                    Coord(firstAxisPos.getX() + pos * (width / (nbAxis - 1)), firstAxisPos.getY());
            } else {
        ...
        if (parallelAxis.contains(selectedProp)) {
            ...
            if (typeName == "string") {
                axis = new NominalParallelAxis(coord, height, maxCaptionWidth, graphProxy,
                                               selectedProp, axisColor, rotationAngle,
                                               captionPosition);
            } else if (typeName == "int" || typeName == "double") {
                axis = new QuantitativeParallelAxis(coord, height, maxCaptionWidth, graphProxy,
                                                    selectedProp, true, axisColor, rotationAngle,
                                                    captionPosition);
            }
        }
            ...
            if (!graphProxy->highlightedEltsSet() || graphProxy->isDataSelected(dataId)) {
                ...
                if (graphProxy->getDataLocation() == ElementType::NODE) {
                    axisPointsGraphShape->setNodeValue(
                        n, graphProxy->getPropertyValueForData<IntegerProperty, IntegerType>(
                               "viewShape", dataId));
                } else {
                ...
                axisPointsGraphLabels->setNodeValue(
                    n, graphProxy->getPropertyValueForData<StringProperty, StringType>("viewLabel",
                                                                                       dataId));
                axisPointsGraphColors->setNodeValue(
                    n, graphProxy->getPropertyValueForData<ColorProperty, ColorType>("viewColor",
                                                                                     dataId));
