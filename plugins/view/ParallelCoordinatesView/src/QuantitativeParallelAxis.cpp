...
void QuantitativeParallelAxis::setAxisLabels() {
    ...
    if (propertyMin >= INT_MIN && propertyMax <= INT_MAX) {
        if (getAxisDataTypeName() == "double") {
            for (uint n : graphProxy->getDataIterator()) {
                double val = graphProxy->getPropertyValueForData<DoubleProperty, DoubleType>(
                    getAxisName(), n);
    ...
    if (getAxisDataTypeName() == "double" && realScale) {
        ...
        ullong incrementStep = (max - min) / DEFAULT_NB_AXIS_GRAD;

        if (incrementStep < 1) {
            incrementStep = 1;
        }

        glQuantitativeAxis->setAxisParameters(min, max, incrementStep, GlAxis::RIGHT_OR_ABOVE,
                                              true);
        integerScale = true;
    }
...
void QuantitativeParallelAxis::computeBoxPlotCoords() {
    ...
    for (uint dataId : graphProxy->getDataIterator()) {
        ...
        if (getAxisDataTypeName() == "double") {
            value = graphProxy->getPropertyValueForData<DoubleProperty, DoubleType>(getAxisName(),
                                                                                    dataId);
        } else {
            value = double(graphProxy->getPropertyValueForData<IntegerProperty, IntegerType>(
                getAxisName(), dataId));
        }
...
double QuantitativeParallelAxis::getAssociatedPropertyMinValue() {
    if (graphProxy->getGraph()->getRoot() == graphProxy->getGraph()) {
        ...
        for (uint dataId : graphProxy->getDataIterator()) {
            ...
            if (getAxisDataTypeName() == "double") {
                propertyValue = graphProxy->getPropertyValueForData<DoubleProperty, DoubleType>(
                    getAxisName(), dataId);
            } else {
...
Coord QuantitativeParallelAxis::getPointCoordOnAxisForData(const uint dataIdx) {
    ...
    if (getAxisDataTypeName() == "double") {
        value =
            graphProxy->getPropertyValueForData<DoubleProperty, DoubleType>(getAxisName(), dataIdx);
...
std::string QuantitativeParallelAxis::getTopSliderTextValue() {
    if (getAxisDataTypeName() == "int" || integerScale) {
        ...
        if (glQuantitativeAxis->hasAscendingOrder()) {
            return getStringFromNumber(val == glQuantitativeAxis->getAxisMaxValue() ? val
                                                                                    : val - 1);
        } else {
