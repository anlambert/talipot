...
void SOMView::drawPreviews() {
    ...
    for (const auto &p : propertiesName) {
        ...
        double maximumDisplayed = inputSample.isUsingNormalizedValues()
                                      ...
                                      : maxValue;
        auto *SOMPrevComp = new SOMPreviewComposite(previewCoord, previewSize, p, colorProperty,
                                                    som, this->properties->getPropertyColorScale(p),
...
void SOMView::buildSOMMap() {
    ...
    if (conn == "4") {
        ...
        std::cerr << __PRETTY_FUNCTION__ << ":" << __LINE__ << " " << "Connectivity not mannaged"
                  << std::endl;
        return;
    }
...
void SOMView::computeMapping() {
    ...
    Size graphDiffSize(
        graphMinSize.getW() == graphMaxSize.getW() ? 1.
                                                   : (graphMaxSize.getW() - graphMinSize.getW()),
        graphMinSize.getH() == graphMaxSize.getH() ? 1.
    ...
    for (const auto &it : mappingTab) {
        ...
        for (auto n : it.second) {
            ...
            if (mt == SOMPropertiesWidget::NoSizeMapping || graphMaxSize == graphMinSize) {
                ...
                nodeSize.set(minElementWidth + ((realSize.getW() - graphMinSize.getW()) /
                                                (graphDiffSize.getW())) *
...
void SOMView::getPreviewsAtViewportCoord(int x, int y, std::vector<SOMPreviewComposite *> &result) {
    ...
    previewWidget->scene()->selectEntities(RenderingEntities, x, y, 0, 0, nullptr,
                                           selectedEntities);
