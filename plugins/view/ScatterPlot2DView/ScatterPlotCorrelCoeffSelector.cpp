    ...
    if (e->type() == QEvent::MouseMove) {
        ...
        if (polygonEdit.empty()) {
            ...
            Coord pointerGlViewportCoord =
                Coord(glWidget->screenToViewport(me->pos().x()),
        ...
        if (!polygonEdit.empty()) {
            ...
            if (selectedAction == deletePoly) {
                ...
                for (size_t i = 0;
                     i < polygonsToNodesSubsetAndCorrelCoeff[selectedPolygon].first.size(); ++i) {
                    for (size_t j = i + 1;
                         j < polygonsToNodesSubsetAndCorrelCoeff[selectedPolygon].first.size();
                         ++j) {
...
void ScatterPlotCorrelCoeffSelector::getPolygonAndPointUnderPointerIfAny(
    ...
    for (auto *polygon : polygons) {
        selectedPolygonPoint =
            polygon->getPolygonVertexUnderPointerIfAny(pointerViewportCoord, camera);
