            ...
            if (!polygonName.empty()) {
                ...
                if (!data.empty()) {
                    ...
                    composite->addGlEntity(
                        new GlComplexPolygon(data, Color(0, 0, 0, 50), Color(0, 0, 0, 255)),
                        polygonName);
                    data.clear();
                    currentVector.clear();
                }
                ...
                for (const auto &polygonSide : polygonSides) {
                    ...
                    for (auto it = (++polygonSide.begin()); it != polygonSide.end(); ++it) {
                        if ((((*it)[1] < nodePos[1] && lastCoord[1] >= nodePos[1]) ||
                            ...
                            oddNodes ^=
                                ((*it)[0] + (nodePos[1] - (*it)[1]) / (lastCoord[1] - (*it)[1]) *
                                                (lastCoord[0] - (*it)[0]) <
                                 nodePos[0]);
                        }
                    ...
                    if (oddNodes) {
                        ...
                        polygon->setFillColor(
                            graph->getColorProperty("viewColor")->getNodeValue(n));
                    ...
                    if (geocodingResults.size() > 1) {
                        ...
                        if (!addressSelectionDialog->pickFirstResult()) {
                            ...
                            addressSelectionProxy->setPos(
                                width() / 2 -
                                    addressSelectionProxy->sceneBoundingRect().width() / 2,
                                height() / 2 -
                                    addressSelectionProxy->sceneBoundingRect().height() / 2);
...
void GeographicViewGraphicsView::centerView() {
    ...
    if (qgvMap->geoView()->isVisible()) {
        if (!nodeLatLng.empty()) {
            ...
            QGV::GeoRect bounds(minLatLng.first, minLatLng.second, maxLatLng.first,
                                maxLatLng.second);
...
void GeographicViewGraphicsView::switchViewType() {
    ...
    if (viewType != GeographicView::Globe && geoLayoutComputed) {
        ...
        if (!planisphereEntity) {
            ...
            planisphereEntity =
                new GlSphere(Coord(0., 0., 0.), 50., planisphereTextureId, 255, 0, 0, 90);
            ...
            *geoLayoutBackup = *geoLayout;
            ...
            for (auto e : graph->edges()) {
                ...
                for (uint i = 0; i < bendsNumber; ++i) {
                    Coord tmp =
                        srcCoord + ((tgtCoord - srcCoord) / (bendsNumber + 1.f)) * (i + 1.f);
