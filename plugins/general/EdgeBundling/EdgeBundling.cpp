...
void updateLayout(node src, edge e, Graph *graph, LayoutProperty *layout,
    ...
    // if source and target nodes are at the same position, don't set bends to avoid visual
    // artifacts when rendering the graph
...
bool EdgeBundling::run() {
    ...
    for (uint iteration = 0; iteration < MAX_ITER; iteration++) {
        ...
        while (!vertexCoverGraph->isEmpty()) {
            ...
            if (((i % 10) == 0) && (pluginProgress->progress(i, oriGraph->numberOfEdges()) !=
