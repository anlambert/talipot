...
int CustomTreeView::sizeHintForColumn(int col) const {
    ...
    while (index.isValid()) {
        if (viewport()->rect().contains(visualRect(index))) {
            ...
            option = viewOptions();
            hint = qMax(hint, visualRect(index).x() +
                                  itemDelegate(index)->sizeHint(option, index).width());
#endif
        }
...
GraphHierarchiesEditor::GraphHierarchiesEditor(QWidget *parent)
    ...
    connect(_ui->actionCreate_panel, &QAction::triggered, this,
            &GraphHierarchiesEditor::createPanel);
...
void GraphHierarchiesEditor::contextMenuRequested(const QPoint &p) {
    ...
    if (_contextIndex.isValid()) {
        _contextGraph =
            _contextIndex.data(tlp::GraphHierarchiesModel::GraphRole).value<tlp::Graph *>();
        QMenu menu;
        ...
        menu.addSeparator();
        QMenu *deleteMenu =
            menu.addMenu(FontIcon::icon(MaterialDesignIcons::DeleteOutline), "Delete");
        if (_contextGraph->getRoot() != _contextGraph) {
        ...
        selectMenu->addAction("graph elements", [this] {
            selectGraphElements(_contextGraph, true, true, true, true);
        });
        selectMenu->addAction(
            "graph nodes", [this] { selectGraphElements(_contextGraph, true, false, true, true); });
        selectMenu->addAction(
            "graph edges", [this] { selectGraphElements(_contextGraph, false, true, true, true); });

        QMenu *addToSelMenu = menu.addMenu(addToSelectionIcon(), "Add to selection");

        addToSelMenu->addAction("graph elements", [this] {
            selectGraphElements(_contextGraph, true, true, true, false);
        });
        addToSelMenu->addAction("graph nodes", [this] {
            selectGraphElements(_contextGraph, true, false, true, false);
        });
        addToSelMenu->addAction("graph edges", [this] {
            selectGraphElements(_contextGraph, false, true, true, false);
        });

        QMenu *removeFromSelMenu = menu.addMenu(
            FontIcon::icon(MaterialDesignIcons::SelectionRemove), "Remove from selection");

        removeFromSelMenu->addAction("graph elements", [this] {
            selectGraphElements(_contextGraph, true, true, false, false);
        });
        removeFromSelMenu->addAction("graph nodes", [this] {
            selectGraphElements(_contextGraph, true, false, false, false);
        });
        removeFromSelMenu->addAction("graph edges", [this] {
            selectGraphElements(_contextGraph, false, true, false, false);
        });
...
void GraphHierarchiesEditor::currentChanged(const QModelIndex &index, const QModelIndex &previous) {
    if (synchronized() && index.isValid() && index.internalPointer()) {
        ...
        _contextGraph = index.data(tlp::Model::GraphRole).value<tlp::Graph *>();
        disconnect(_ui->hierarchiesTree->selectionModel(), &QItemSelectionModel::currentChanged,
                   this, &GraphHierarchiesEditor::currentChanged);
        _model->setCurrentGraph(_contextGraph);
        connect(_ui->hierarchiesTree->selectionModel(), &QItemSelectionModel::currentChanged, this,
                &GraphHierarchiesEditor::currentChanged);
        _contextGraph = nullptr;
