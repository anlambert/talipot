...
class SearchOperator {
  ...
  public:
    virtual ~SearchOperator() = default;
...
SearchWidget::SearchWidget(QWidget *parent)
    ...
    STRING_OPERATORS << new StringEqualsOperator << new StringDifferentOperator << nullptr
                     << nullptr << nullptr << nullptr << new StartsWithOperator
...
void SearchWidget::setGraph(Graph *g) {
    if (g != nullptr) {
        // Force creation of viewSelection to ensure we have at least one boolean property existing
        // in the graph
    ...
    if (_ui->tableWidget->isVisible()) {
        ...
        if (isNumericComparison()) {
            ...
            DataTypeSerializer *serializer =
                DataSet::typenameToSerializer(talipotData->getTypeName());

            if (serializer == nullptr) {
                qCritical() << "no type serializer found for \""
                            << talipotData->getTypeName().c_str()
                            << "\", please report this as a bug";
#ifdef NDEBUG
                delete stringProp;
                return;
