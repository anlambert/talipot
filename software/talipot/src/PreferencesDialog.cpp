...
PreferencesDialog::PreferencesDialog(QWidget *parent)
    ...
    for (int i = 0; i < _ui->graphDefaultsTable->rowCount(); ++i) {
        QString sectionText = _ui->graphDefaultsTable->model()
                                  ->headerData(i, Qt::Vertical, Qt::DisplayRole)
                                  .toString();
...
void PreferencesDialog::writeSettings() {
    ...
    if (Settings::defaultShape(tlp::ElementType::EDGE) !=
        ...
        Settings::setDefaultShape(
            tlp::ElementType::EDGE,
            int(model->data(model->index(2, 1)).value<EdgeShape::EdgeShapes>()));
    ...
    if (Settings::defaultLabelColor() != model->data(model->index(4, 0)).value<tlp::Color>()) {
        ...
        if (applyDrawingDefaults) {
            setDefaultNodeValueInProperty<ColorProperty>("viewLabelColor",
                                                         Settings::defaultLabelColor(), graphPush);
...
void PreferencesDialog::cellChanged(int row, int column) {
    if (row >= 3) {
        ...
        QAbstractItemModel *model = _ui->graphDefaultsTable->model();
        model->setData(model->index(row, column == 0 ? 1 : 0),
...
void PreferencesDialog::resetToTalipotDefaults(int row, int updateMode) {
    ...
    case 0: // default color
        if (updateMode == RESET_NODE) {
            model->setData(model->index(0, 0), QVariant::fromValue<tlp::Color>(
                                                   ViewSettings::ApplicationDefault::NodeColor));
        } else {
            model->setData(model->index(0, 1), QVariant::fromValue<tlp::Color>(
                                                   ViewSettings::ApplicationDefault::EdgeColor));
        }
    ...
    case 1: // default size
        if (updateMode == RESET_NODE) {
            model->setData(model->index(1, 0), QVariant::fromValue<tlp::Size>(
                                                   ViewSettings::ApplicationDefault::NodeSize));
        } else {
            model->setData(model->index(1, 1), QVariant::fromValue<tlp::Size>(
                                                   ViewSettings::ApplicationDefault::EdgeSize));
        }
    ...
    case 3: // default selection color
        if (updateMode == RESET_NODE) {
            model->setData(
                model->index(3, 0),
                QVariant::fromValue<tlp::Color>(ViewSettings::ApplicationDefault::SelectionColor));
        } else {
            model->setData(
                model->index(3, 1),
                QVariant::fromValue<tlp::Color>(ViewSettings::ApplicationDefault::SelectionColor));
    ...
    case 4: // default label color
        if (updateMode == RESET_NODE) {
            model->setData(model->index(4, 0), QVariant::fromValue<tlp::Color>(
                                                   ViewSettings::ApplicationDefault::LabelColor));
        } else {
...
void PreferencesDialog::showGraphDefaultsContextMenu(const QPoint &p) {
    ...
    if (row < 3) {
        ...
        action->setToolTip("Reset the node/edge " + defaultProp +
