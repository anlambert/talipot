...
AlgorithmRunnerItem::AlgorithmRunnerItem(QString pluginName, QWidget *parent)
    ...
    connect(_ui->favoriteCheck, QCheckBoxStateChangedSignal, this,
...
void AlgorithmRunnerItem::run(Graph *g) {
    ...
    for (const ParameterDescription &desc : paramList.getParameters()) {
        ...
        // temporary property
        outPropParam.tmp =
            outPropParam.dest ? outPropParam.dest->clonePrototype(outPropParam.dest->getGraph(), "")
                              : nullptr;
    ...
    if (!result) {
        if (progress->state() == ProgressState::TLP_CANCEL && errorMessage.empty()) {
            ...
            std::string errorMsg =
                "\"" + QStringToTlpString(name()) + "\" algorithm execution failed.";
    ...
    if (result) {
        ...
        for (const OutPropertyParam &opp : outPropertyParams) {
            ...
            if (opp.name == "result" && Settings::isResultPropertyStored()) {
                ...
                std::string storedResultName = algorithm + " - " + originalDataSet.toString() +
                                               "(" + opp.dest->getName() + ")";
...
void AlgorithmRunnerItem::afterRun(Graph *g, const tlp::DataSet &dataSet) {
    ...
    if (PluginsManager::pluginExists<LayoutAlgorithm>(stdName)) {
    ...
    } else if (Settings::isAutomaticCentering() &&
        ...
        if (result) {
            ...
            QMessageBox::information(parentWidget(), "Talipot test result",
                                     tlp::tlpStringToQString(str));
        } else {
            tlp::warning() << str << std::endl;
            QMessageBox::warning(parentWidget(), "Talipot test result",
                                 tlp::tlpStringToQString(str));
        }
    }
}
